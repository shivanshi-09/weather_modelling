...
class TargetDataset(Dataset):
    def __init__(self, cfg, split="train"):
        ...
        self.era5_files = sorted(Path(cfg.era5_dir).glob("*.nc"))
        ...
        self.era5_mean   = float(stats["era5_mean"])
        self.era5_std    = float(stats["era5_std"])
    ...
    def _get_era5(self):
        if self._era5_ds is None: 
            self._era5_ds = xr.open_mfdataset(
                self.era5_files, combine = "by_coords"
            )[self.cfg.era5_var]
    ...
    def __getitem__(self, idx):
        ...
        target = ds[self.cfg.target_var].isel(
            time=t_idx,
            latitude=lat_sl,
            longitude=lon_sl,
        ).values.astype(np.float32)
        ...
        era5 = self._get_era5()
        era5_t = era5.sel(time=ts, method="nearest", tolerance=np.timedelta64(1, "h"))
        era5_patch = era5_t.sel(
            lat=slice(lat.min(), lat.max()),
            lon=slice(lon.min(), lon.max()),
        )
        era5_up = era5_patch.interp(lat=lat, lon=lon, method="linear").values.astype(np.float32)
        era5_up = (era5_up - self.era5_mean) / self.era5_std
        era5_up = np.nan_to_num(era5_up, nan=0.0)

        cond = np.stack([topo, svf, csza, era5_up], axis=0)

        return torch.from_numpy(np.ascontiguousarray(cond)), torch.from_numpy(np.ascontiguousarray(target[None]))
